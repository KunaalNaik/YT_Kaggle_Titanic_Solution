#Working Directory
setwd("D:/FunXExcel Channel/17. Kaggle/GitHub/Kaggle_Titanic_R/3_train_gender_all_numericals")

#Import
df_train <- read.csv("train_gender_all_numericals.csv")
df_test <- read.csv("test_gender_all_numericals.csv")
...
#Replace missing value by mean of Fare - train
table(is.na(df_test$Fare))
df_test$Fare[is.na(df_test$Fare)] <- round(mean(df_test$Fare, na.rm = TRUE))
table(is.na(df_test$Fare))
...
logistic <- glm(Survived ~ Age + Sex + Pclass + SibSp + Parch + Fare,data = df_train , family = binomial)
summary(logistic)
...
write.csv(solution, "submission_train_gender_all_numericals.csv", row.names = F)
